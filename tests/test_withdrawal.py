...
def test_withdraw_roth_last():
    ...
    updated, _ = withdraw_funds(holdings, 50)
    roth = next(h for h in updated if h.account_type == "ROTH")
    assert roth.qty == 100  # not touched yet
...
def test_withdraw_from_hsa_cash_first():
    ...
    updated, withdrawn = withdraw_from_hsa([hsa_cash], 3_000)
...
def test_withdraw_from_hsa_equity_when_cash_insufficient():
    ...
    updated, withdrawn = withdraw_from_hsa([hsa_cash, hsa_eq], 2_000)
    assert withdrawn >= 2_000


def test_withdraw_funds_negative_amount():
    holdings = [_cash()]
    updated, withdrawn = withdraw_funds(holdings, -100)
    assert withdrawn == 0.0


def test_withdraw_from_hsa_break_on_second_cash():
    # Two HSA cash holdings; first covers the full need — second should be skipped (break path).
    ...
    updated, withdrawn = withdraw_from_hsa([hsa1, hsa2], 3_000)
    assert abs(withdrawn - 3_000) < 0.01
    remaining_cash = sum(h.qty for h in updated if h.account_type == "HSA" and h.ticker == "CASH")
    assert abs(remaining_cash - 7_000) < 0.01
